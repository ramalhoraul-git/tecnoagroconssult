        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 lg:flex">
            {navLinks.map((link) => (
              ...
              >
                <a className="text-sm font-medium tracking-[0.08em] text-[#1F2933] transition hover:text-[#1E6B3A]" href={`#${link.id}`}>
                  {link.label}
                </a>
                {link.submenu && (
                  <div className={cn(
                    ...
                    <div className="min-w-[220px] rounded-2xl bg-white p-2 shadow-[0_18px_60px_rgba(0,0,0,0.12)] border border-[#1E6B3A]/10">
                      {link.submenu.map((sub) => (
                        <a key={sub.id} href={`#${sub.id}`} className="block rounded-xl px-4 py-3 text-sm text-[#1F2933] transition hover:bg-[#1E6B3A]/5 hover:text-[#1E6B3A]">
                          {sub.label}
                        </a>
                      ))}
                    </div>
          ...
          {menuOpen ? (
            ...
            >
              <div className="mx-auto flex max-w-7xl flex-col gap-3">
                {navLinks.map((link) => (
                  <div key={link.id}>
                    <a className="block rounded-xl px-3 py-3 text-base font-medium text-[#1F2933] transition hover:bg-[#1E6B3A]/5 hover:text-[#1E6B3A]" href={`#${link.id}`} onClick={() => setMenuOpen(false)}>
                      {link.label}
                    </a>
                    {link.submenu && (
                      <div className="ml-4 space-y-1 border-l-2 border-[#1E6B3A]/20 pl-4">
                        {link.submenu.map((sub) => (
                          <a key={sub.id} href={`#${sub.id}`} className="block rounded-xl px-3 py-2 text-sm text-[#1F2933]/70 transition hover:bg-[#1E6B3A]/5 hover:text-[#1E6B3A]" onClick={() => setMenuOpen(false)}>
                            {sub.label}
                          </a>
                        ))}
                      </div>
      ...
      <main>
        ...
        <section ref={heroRef} id="home" className="relative flex min-h-[100svh] items-center overflow-hidden pt-20">
          <motion.div className="absolute inset-0" style={{ y: videoY, scale: videoScale }}>
            <video className="h-full w-full object-cover" autoPlay muted loop playsInline preload="metadata" poster="https://images.pexels.com/videos/34940811/pexels-photo-34940811.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=1200&w=1600">
          ...
          <motion.div className="relative z-10 mx-auto flex w-full max-w-7xl px-4 sm:px-6 lg:px-8" style={{ y: overlayY }}>
            <div className="max-w-4xl space-y-8 py-12 lg:py-20">
              <motion.div className="inline-flex items-center gap-3 rounded-full border border-[#C9A227]/20 bg-white/8 px-4 py-2 text-[11px] tracking-[0.34em] text-white/75 backdrop-blur-md uppercase" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              ...
              <motion.div className="space-y-4" initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.8 }}>
                <p className="font-display text-5xl tracking-[0.22em] text-white/96 sm:text-6xl lg:text-7xl xl:text-8xl">TECNOAGRO</p>
                <p className="text-sm tracking-[0.32em] text-[#C9A227] uppercase sm:text-base">A Inteligência que Cultiva Legados de Ouro</p>
              </motion.div>

              <motion.h1 className="max-w-4xl font-display text-5xl leading-[1.02] text-white sm:text-6xl lg:text-7xl xl:text-8xl" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.85 }}>
        ...
        <section className="border-y border-white/8 bg-[#0f3520] py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              ...
              <p className="mt-3 text-white/70 max-w-2xl mx-auto">Enquanto outros apenas colhem, você constrói um legado. Cada decisão se torna um ativo.</p>
            </div>

            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {valueMessages.map((item, index) => (
                <motion.div key={index} className="glass-panel p-6 text-center transition hover:-translate-y-2 hover:border-[#C9A227]/30" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeUp}>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              ...
              <p className="mt-6 max-w-3xl mx-auto text-white/70 text-lg leading-8">
                Cada propriedade possui uma história. Nosso papel é transformar essa história em produtividade, rentabilidade e legado com rigor técnico, visão de futuro e presença constante ao lado do produtor.
              </p>
            </div>
        ...
        <section id="estrategias" className="py-24 md:py-32 bg-[#13482A]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading eyebrow="Estratégias" title="O selo de excelência que consolida propriedades mais prósperas e eficientes." description="Nosso método combina ciência aplicada, inteligência agronômica e visão empresarial para organizar a operação, fortalecer a tomada de decisão e valorizar o patrimônio do produtor." center />
        ...
        <section id="diferenciais" className="py-24 md:py-32 bg-[#13482A]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading eyebrow="Diferenciais" title="Porque escolher a TECNOAGRO?" description="Uma experiência premium de consultoria que une presença no campo, inteligência de gestão e tecnologia para elevar a tomada de decisão." center />
            <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {differentiators.map((item, index) => (
                <motion.div key={item.title} className="group glass-panel p-6 transition duration-300 hover:border-[#C9A227]/35 hover:bg-white/8" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.35 }} variants={fadeUp} whileHover={{ y: -8 }} transition={{ duration: 0.35 }}>
        ...
        <section id="servicos" className="py-24 md:py-32 bg-[#13482A]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading eyebrow="Serviços" title="Soluções completas para o agro de alta performance." description="Cada serviço foi organizado para transmitir clareza, escala e amplitude técnica, com a elegância de uma consultoria global e a identidade do campo brasileiro." center />
                ...
                <motion.div key={group.title} className="group glass-panel p-7" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} whileHover={{ y: -6 }}>
                  ...
                  <div className="mt-7 grid gap-3">
                    {group.items.map((item) => (
                      <div key={item} className="rounded-[22px] border border-white/8 bg-white/4 px-4 py-4 text-sm tracking-[0.06em] text-white/82 transition group-hover:border-[#C9A227]/30 group-hover:bg-white/8">
                        {item}
                      </div>
                    ))}
                  </div>
        ...
        <section id="processo" className="py-24 md:py-32 bg-[#13482A]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading eyebrow="Processo" title="Como trabalhamos" description="Uma jornada clara e acompanhada de perto, da leitura da propriedade à geração de resultados mensuráveis." center />
            <div className="relative mt-16">
              <motion.div className="absolute left-6 right-6 top-7 hidden h-px origin-left bg-gradient-to-r from-transparent via-[#C9A227]/80 to-transparent md:block" initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 1.1, ease: "easeOut" }} />
              <div className="grid gap-4 md:grid-cols-5">
                {processSteps.map((step, index) => (
                  <motion.div key={step} className="relative glass-panel p-6 text-center" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeUp}>
                    <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border border-[#C9A227]/30 bg-[#C9A227]/12 font-display text-2xl text-[#C9A227]">0{index + 1}</div>
        ...
        <section id="galeria" className="py-24 md:py-32 bg-[#13482A]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading eyebrow="Galeria Premium" title="O campo em escala cinematográfica." description="Uma curadoria visual com drone, colheita, pulverização, plantio, máquinas e consultoria em movimento." center />
            <div className="mt-12 relative overflow-hidden rounded-[32px] border border-white/10 bg-white/5">
              <div className="relative aspect-[16/9] overflow-hidden">
                ...
                <button onClick={() => setCurrentGalleryIndex((prev) => (prev - 1 + galleryItems.length) % galleryItems.length)} className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full bg-black/40 p-3 text-white backdrop-blur-md transition hover:bg-black/60" aria-label="Anterior">
                  <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                </button>
                <button onClick={() => setCurrentGalleryIndex((prev) => (prev + 1) % galleryItems.length)} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-black/40 p-3 text-white backdrop-blur-md transition hover:bg-black/60" aria-label="Próximo">
              ...
              <div className="flex justify-center gap-2 p-4 bg-black/20">
                {galleryItems.map((_, index) => (
                  <button key={index} onClick={() => setCurrentGalleryIndex(index)} className={cn("h-2 rounded-full transition-all", index === currentGalleryIndex ? "w-8 bg-[#C9A227]" : "w-2 bg-white/30")} aria-label={`Ver imagem ${index + 1}`} />
                ))}
              </div>
            </div>
          </div>
        ...
        <section id="visao" className="py-24 md:py-32 bg-[#13482A]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <motion.div className="relative overflow-hidden rounded-[40px] border border-white/10" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.35 }} variants={fadeUp}>
              <img src="https://images.pexels.com/photos/26256452/pexels-photo-26256452.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=1600&w=2400" alt="Colheitadeira em grande área agrícola vista de cima" className="h-[34rem] w-full object-cover object-center" />
              ...
              <motion.div className="absolute left-0 top-0 h-full w-full px-6 py-10 sm:px-10 lg:px-16 lg:py-16" initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true, amount: 0.4 }}>
                <div className="flex h-full max-w-2xl flex-col justify-end">
                  <p className="section-kicker">Visão de Futuro</p>
                  <h2 className="mt-4 font-display text-4xl leading-tight text-white md:text-5xl lg:text-6xl">O agro do amanhã começa hoje.</h2>
                  <p className="mt-5 max-w-xl text-base leading-8 text-white/78 md:text-lg">Enquanto muitos enxergam apenas uma safra, nós enxergamos gerações. Nossa missão é construir propriedades mais produtivas, sustentáveis e valorizadas.</p>
                </div>
              </motion.div>
            </motion.div>
          </div>
        ...
        <section id="resultados" className="py-28 md:py-36 bg-[#13482A]">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading eyebrow="Depoimentos" title="A confiança de quem produz em escala." description="A percepção do produtor confirma a qualidade da consultoria e o impacto da parceria no dia a dia da fazenda." center />
            <div className="mt-14 grid gap-4 lg:grid-cols-[1fr_1.18fr]">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    {testimonials.map((_, index) => (
                      <button key={index} type="button" className={cn("h-2.5 rounded-full transition-all", index === activeTestimonial ? "w-8 bg-[#C9A227]" : "w-2.5 bg-white/30")} onClick={() => setActiveTestimonial(index)} aria-label={`Ver depoimento ${index + 1}`} />
                    ))}
                  </div>
                </div>
              ...
              <GlassPanel className="relative overflow-hidden p-8 lg:p-10">
                ...
                <div className="relative">
                  ...
                  <div className="mt-10 rounded-[28px] border border-white/10 bg-white/4 p-6 text-sm leading-7 text-white/72">Nosso foco é ampliar produtividade com segurança técnica e uma gestão capaz de proteger o patrimônio do produtor.</div>
                </div>
              </GlassPanel>
            </div>
          </div>
                ...
                <form className="mt-8 grid gap-4" onSubmit={submitContact}>
                  {[
                    ...
                    <label key={field.name} className="grid gap-2 text-sm text-white/74">
                      ...
                      <input className="premium-input" type={field.type} name={field.name} placeholder={field.placeholder} value={formState[field.name as keyof typeof formState]} onChange={(event) => setFormState((current) => ({ ...current, [field.name]: event.target.value }))} required={field.name !== "area"} />
                    </label>
                  ))}
